...
function test_nop {
  ...
  sudo ./build/app/nf \
        ...
        --no-huge \
        --no-shconf -- \
        --eth-dest 0,$(echo $MAC_1) \
        --eth-dest 1,$(echo $MAC_2) \
        --lan-device 0 \
        --wan-device 1 \
        >/dev/null 2>&1 &
  NF_PID=$!
  ...
  sudo ip netns exec lan tcpdump -i test_lan -w ~/packets_ping.pcap > /dev/null & sleep 10
  sudo ip netns exec lan ping -c 5 10.0.0.2
